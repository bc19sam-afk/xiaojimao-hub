...
// recordIngest 是真实执行的。MOCK=true → 跳过 finishOAuth 的快照 fail-closed 门、
...
test('recordIngest：认不出账号（accountId 空）→「未能确认」提示，非「交过了」', async () => {
  ...
  const r = await collect.finishOAuth(user(9001), 'grok', 'https://app/callback?state=s-empty')
...
test('recordIngest：真重复（有 accountId + duplicate）→「这个号交过了」', async () => {
  ...
  const r = await collect.finishOAuth(user(9002), 'grok', 'https://app/callback?state=s-dup')
  assert.equal(r.ok, false)
