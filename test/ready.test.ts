...
test('ready：全新迁移到最新版的库 → true', () => {
  assert.equal(checkReady(), true)
})

// ② schema 版本落后 → 未就绪。直接改库里的 schema_version（另开连接写，应用那条连接读得到——
//    同一文件、WAL 下新事务可见已提交的写），验证探针读的是**当下**的库状态而非启动时的缓存。
test('ready：schema_version 落后于 LATEST_VERSION → false', () => {
...
test('ready：schema_version 超前于 LATEST_VERSION → false（严格相等，区别于启动期守卫）', () => {
  const w = new DatabaseSync(dbPath)
  w.exec('PRAGMA busy_timeout = 5000')
  w.prepare('UPDATE schema_version SET version = ?').run(LATEST_VERSION + 1)
  w.close()
...
//    探针绝不能因为被探测对象坏了而 500。
test('ready：schema_version 表缺失 → false 不抛', () => {
  const w = new DatabaseSync(dbPath)
  w.exec('PRAGMA busy_timeout = 5000')
  ...
  const unmute = muteError()
  try {
    assert.doesNotThrow(() => checkReady())
    assert.equal(checkReady(), false)
  } finally {
    unmute()
  }
...
test('ready：schema_version 多行（readSchemaVersion 抛）→ false 不抛', () => {
  const w = new DatabaseSync(dbPath)
  w.exec('PRAGMA busy_timeout = 5000')
